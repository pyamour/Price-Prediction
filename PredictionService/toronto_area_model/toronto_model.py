...
from keras.layers import Dense, Dropout, Activation, LeakyReLU, MaxoutDense
...
def fully_connected_Model(num_features):
    model = Sequential()

    model.add(Dense(input_dim=num_features, units=512, kernel_initializer='glorot_uniform'))
    model.add(BatchNormalization())
